...
> ⚠️ **关于Netlify部署的重要说明**：你可能会注意到在Netlify上部署的站点显示为空白页面，并有资源加载错误。这是正常且预期的情况，因为本模板设计为将内容发布到GitHub Pages，而不是直接在Netlify上。Netlify仅用于其身份验证服务。要查看你的实际网站，请使用你的GitHub Pages URL。
...
2. 同样需要更新`public/identity.html`中的相同URL：

```javascript
// 定义站点URL
const NETLIFY_SITE = "your-netlify-site-name.netlify.app"; // 替换为您的Netlify站点名称
```

3. 然后更新`src/content/config.json`添加管理页面URL：
...
如果遇到Netlify身份验证问题（例如设置密码或登录困难）：

1. 收到邀请邮件时，确保点击链接并打开`identity.html`页面
2. 如果尝试设置密码时看到空白页面或错误：
   - 手动访问你的Netlify站点URL加上`/identity.html`（例如：`your-site-123456.netlify.app/identity.html`）
   - 使用"打开登录窗口"按钮手动触发身份验证
   - 从邮件URL中复制邀请令牌（invite_token），粘贴到浏览器地址栏的`/identity.html#invite_token=`后面

3. 如果问题持续，检查浏览器控制台的具体错误，并确保配置中的所有URL与实际Netlify域名匹配
