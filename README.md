...
1. 在`public/admin/index.html`文件中，找到并修改：

```javascript
// 定义Netlify站点URL
const NETLIFY_SITE = "your-netlify-site-name.netlify.app"; // 替换为你的Netlify站点名称
const API_URL = `https://${NETLIFY_SITE}/.netlify/identity`;
```

2. 在`public/admin/config.yml`文件中，找到并修改：

```yml
backend:
  name: git-gateway
  branch: main
  site_domain: your-netlify-site-name.netlify.app  # 不需要https://前缀
  base_url: https://your-netlify-site-name.netlify.app  # 需要https://前缀
```

⚠️ **重要提示：** 这一步非常关键！如果不正确配置Netlify站点URL，身份验证将会失败，导致无法登录CMS。请确保将所有出现的`your-netlify-site-name.netlify.app`替换为你自己的Netlify站点域名（如"my-blog-123456.netlify.app"）。
